...
const updateProfile = async (req, res) => {
    try {
        const { firstname, lastname, dateOfBirth, username, profilePicture } = req.body;
        ...
        if (req.localProfile) {
            updateData.profileImage = req.localProfile;
            updateData.profilePicture = req.localProfile.url;

            // Delete old profile image from local storage if exists
            const currentUser = await User.findById(req.user.id);
            if (currentUser.profileImage && currentUser.profileImage.path) {
                try {
                    await deleteImage(currentUser.profileImage.path);
                } catch (deleteError) {
